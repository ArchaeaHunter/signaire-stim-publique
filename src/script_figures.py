...
def plot_treemap_domains_subdomains(df,save_dir) : 
    ...
    color_scale = px.colors.sample_colorscale("oranges", [n/(n_colors -1) for n in range(n_colors)])
    ...
    fig.write_html(os.path.join(save_dir, "treemap_domains.html"))
...
def plot_cumsum_dates(df,save_dir) : 
    ...
    fig.write_html(os.path.join(save_dir, "signes_au_cours_du_temps.html"))
...
def plot_treemap_domain_year(df,save_dir) : 
    ...
    fig.write_html(os.path.join(save_dir, "treemap_domains_annee.html"))
...
def plot_treemap_year_domain(df,save_dir) : 
    ...
    fig.data[0].textinfo = 'label+text+value'
    fig.write_html(os.path.join(save_dir, "treemap_annee_domains.html"))
